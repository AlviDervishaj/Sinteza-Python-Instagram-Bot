...
def interact_with_user(
    ...
    start_time = time()
    blogger_competitor = blogger_competitor if blogger_competitor != "" else ""
    profile_data, skipped, skipped_per_competitor = profile_filter.check_profile(device, username, False, my_username, blogger_competitor)
    session_state.skipped_per_competitor.update(skipped_per_competitor if skipped_per_competitor is not None else {})
    logger.error(skipped_per_competitor if skipped_per_competitor is not None else "")
    ...
    if can_like(session_state, likes_percentage):
        ...
        for i in range(len(photos_indices)):
            ...
            if opened_post_view is None:
                save_crash(device, session_state)
                continue
    ...
    if not inside_working_hours:
        ...
        if args.scrape_to_file is not None:
            ...
            if session_state.check_limit(
                ...
                can_continue = False
            if session_state.check_limit(
                limit_type=session_state.Limit.SKIPPED_LISTS, output=False
            ):
                logger.info(
                    "Reached total skipped limit, finish.",
                    extra={"color": f"{Fore.CYAN}"},
                )
                can_continue = False
            if session_state.check_limit(
