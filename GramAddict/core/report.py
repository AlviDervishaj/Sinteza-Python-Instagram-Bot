...
def print_full_report(sessions, scrape_mode):
    if len(sessions) > 1:
        for index, session in enumerate(sessions):
            ...
            if scrape_mode is None:
                ...
                logger.info(
                    f"Total users skipped: {session.skipped_per_competitor}",
                    extra={"color": f"{Style.BRIGHT}{Fore.YELLOW}"},
                )
                logger.info(
                    f"Total lists skipped: {session.skipped_lists_per_competitor}",
                    extra={"color": f"{Style.BRIGHT}{Fore.YELLOW}"},
                )
                logger.info(
    ...
    for session in sessions:
        ...
        for competitor, count in session.skipped_per_competitor.items():
            logger.info(
                f"-- Skipped {count} user(s) for {competitor} --",
                extra={"color": f"{Style.BRIGHT}{Fore.YELLOW}"},
            )
    if scrape_mode is None:
        logger.info(
