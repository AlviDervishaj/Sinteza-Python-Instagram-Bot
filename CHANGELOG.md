...
## 3.3.0b29 (2022-11-02)
### New Features
- welcoming new followers with `welcoming: 3-5` and control how many skips before ending the job with `max-welcoming-skips: 5-10`
- check if the chat is empty before welcoming `check-chat-before-welcoming: bool` if the chat is present the user won't be welcomed
- it uses the sentences inside `pm_welcome.txt` in your account folder
- disable activity (stories and posts) of an account just followed (works only for not private account, of course)
  new arguments are: `mute-posts-after-follow: bool` and `mute-stories-after-follow: bool`
- autogen mode in unfollow-from-file that allows to autogenerate the list of usernames to unfollow that match the unfollow-delay value
  to use it pass the `autogen` special word instead of the filename in `unfollow-from-file: [autogen 3-4]`
- `unfollow-skip-limit: int` allows to set the number skips in the username list before ending the job
- rotate ip by switching on/off airplane mode `rotate-ip: bool`
- specify how many hours since the last check have to pass before checking again `can-recheck-after: int`
- `timeout-startup: int` sets the initial timeout in minutes before starting job activities
- `change-source-if-crash: bool` if there is a local crash, don't try again with the same target but move to the next one
- search for the account among the non-logged-in accounts and possibly attempt to log in by putting in the password `password: str`
- with `relog-after-block: bool` you can pause the script by dislodging the account and re-logging it after the minutes specified in `relog-delay: int`
### Performance improvements
- wait more for the sending icon in PM to disappear
- speed up scrolling over list (time has been halved!)
- checks if account exists before putting into the list in `autogen` mode
- returns checked time and filtered time
### Bug fixes
- unwanted sleeping delay in click_retry function
- was missing the `exists` parameter in the json file, so from now it will be added for interactions
- don't add username in interacted_users.json when the user is skipped
- welcome plugin was filtering users which is not needed
- bot will check for block after double-click on posts and single click if failed
- config.yml load was not working for module-mode
### Others
- tested_ig_version has been moved from bot_flow.py to version.py
- `gramaddict dump` console command now allows to specify the device name and don't kill uia2 demon
- logic has been simplified where possible
- jobs are divided in 3 parts: actions, special and analytics
- store device serial in the device_id property
- clean dummy data in interacted_users.json which was added when the bot skips a user
- welcome plugin has corrupted some data, so it has been fixed and the bot tries to deal with it somehow
- wait more when looking for the action bar
## 3.2.2 (2022-04-28)
### Fix
